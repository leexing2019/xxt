...
// 展开的配置区块
const activeSection = ref<string | null>('baidu-ocr')
...
const speechConfigured = computed(() => !!formData.baiduSpeechApiKey && !!formData.baiduSpeechSecretKey)
...
async function loadConfig() {
  ...
  try {
    ...
    if (config) {
      formData.baiduOcrApiKey = config.baiduOcrApiKey || ''
      formData.baiduOcrSecretKey = config.baiduOcrSecretKey || ''
      formData.baiduSpeechAppId = config.baiduSpeechAppId || ''
      formData.baiduSpeechApiKey = config.baiduSpeechApiKey || ''
      formData.baiduSpeechSecretKey = config.baiduSpeechSecretKey || ''
    }
  } catch (error) {
...
// 测试百度 OCR API 连接
async function testBaiduOcrApi() {
  if (!formData.baiduOcrApiKey || !formData.baiduOcrSecretKey) {
    ElMessage.warning('请先填写 OCR API Key 和 Secret Key')
    return
  }

  const result = await testBaiduApiConnection(formData.baiduOcrApiKey, formData.baiduOcrSecretKey)
  if (result.success) {
...
// 测试百度语音 API 连接
async function testBaiduSpeechApi() {
  if (!formData.baiduSpeechApiKey || !formData.baiduSpeechSecretKey) {
    ElMessage.warning('请先填写语音 API Key 和 Secret Key')
    return
  }

  const result = await testBaiduApiConnection(formData.baiduSpeechApiKey, formData.baiduSpeechSecretKey)
  if (result.success) {
    ElMessage.success(result.message)
  } else {
    ElMessage.error(result.message)
...
async function saveConfig() {
  ...
  try {
    const configToSave: ApiConfig = {
      baiduOcrApiKey: formData.baiduOcrApiKey,
      baiduOcrSecretKey: formData.baiduOcrSecretKey,
      baiduSpeechAppId: formData.baiduSpeechAppId,
      baiduSpeechApiKey: formData.baiduSpeechApiKey,
      baiduSpeechSecretKey: formData.baiduSpeechSecretKey,
      drugApiBaseUrl: '',
      drugApiKey: ''
    }
...
<template>
  <div class="api-settings">
    ...
    <el-card class="config-section">
      <template #header>
        <div class="section-header" @click="toggleSection('baidu-ocr')">
          <div class="section-title">
            <el-icon class="section-icon"><Camera /></el-icon>
            <span>百度 OCR API 配置</span>
          </div>
          <div class="section-status">
            ...
            <el-icon class="expand-icon">
              <ArrowDown v-if="activeSection === 'baidu-ocr'" />
              <ArrowRight v-else />
            </el-icon>
          </div>
        </div>
      ...
      <el-collapse-transition>
        <div v-if="activeSection === 'baidu-ocr'" class="section-content">
          ...
          <el-form label-position="top" class="config-form">
            ...
            <el-button type="primary" @click="testBaiduOcrApi" size="default">
    ...
    <el-card class="config-section">
      <template #header>
        <div class="section-header" @click="toggleSection('baidu-speech')">
          <div class="section-title">
            <el-icon class="section-icon"><Mic /></el-icon>
            <span>百度语音 API 配置</span>
          </div>
          <div class="section-status">
            ...
            <el-icon class="expand-icon">
              <ArrowDown v-if="activeSection === 'baidu-speech'" />
              <ArrowRight v-else />
            </el-icon>
          </div>
        </div>
      ...
      <el-collapse-transition>
        <div v-if="activeSection === 'baidu-speech'" class="section-content">
          <el-alert type="info" :closable="false" class="guide-box">
            <template #title>
              <div class="guide-title">
                <span>💡</span> 如何申请
          ...
          <el-form label-position="top" class="config-form">
            ...
            <el-button type="primary" @click="testBaiduSpeechApi" size="default">
              <el-icon><Connection /></el-icon>
              测试连接
            </el-button>
          </el-form>
    ...
    <el-alert type="warning" :closable="false" class="tips-section">
      ...
      <div class="tips-content">
        ...
        <p><el-icon><Check /></el-icon> OCR 和语音配置相互独立，可分别管理</p>
      </div>
    </el-alert>
  </div>
</template>
