...
export async function getAllUserComplianceStats(date?: string): Promise<UserComplianceStats[]> {
  ...
  try {
    ...
    return (todayData || []).map((item: any) => ({
      ...
      week_compliance: Number(weekData?.find((w: any) => w.user_id === item.user_id)?.compliance_rate) || 0,
      month_compliance: Number(monthData?.find((m: any) => m.user_id === item.user_id)?.compliance_rate) || 0
    }))
  } catch (error) {
 ...
 */
export async function getAverageComplianceRate(days: number = 7): Promise<number> {
  ...
  try {
    ...
    const validRates = (data || [])
      .map((item: any) => Number(item.compliance_rate) || 0)
      .filter((rate: number) => rate > 0)

    if (validRates.length === 0) return 0

    const avg = validRates.reduce((sum: number, rate: number) => sum + rate, 0) / validRates.length
